...
It's 6 AM and Elf McSkidy is clocking-in to The Best Festival Company's SOC headquarters to begin his watch over TBFC's infrastructure. After logging in, Elf McSkidy proceeds to read through emails left by Elf McSkidy during the nightshift.
...
ip.dst 	Show all packets that are destined to the specified IP address 	ip src == 192.168.1.1
tcp/udp.port 	Show all packets that are sent via the protocol and port specifiyed 	tcp.port == 22 / udp.port == 67
...
Analyse "pcap3.pcag" and recover Christmas!
