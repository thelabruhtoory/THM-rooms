**Run the following to store information as environmentatl variables**
...
```
 ...
 export alt_url=http://:/
 export attack_ip=$(ip a s | grep tun0 | grep inet | cut -f2 -d "t" | cut -f2 -d " " | cut -f1 -d "/")
 export net_subnet=$(ip a s | grep tun0 | grep inet | cut -f2 -d "t" | cut -f2 -d " " | cut -f2 -d "/")
